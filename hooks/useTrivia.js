...
export default function useTrivia(game, onFinish) {
  ...
  const update = (selectedOption, ms) => {
    setState((prev) => {
      ...
      if (game.questions.length <= currentIndex) {
        const score = successfulChoices * 1000 + currentQuestionTimeAward;
  ...
  return {
    ...
    totalPossibleScore: game.questions.length * 1000,
    update,
  };
}
